...
class AttributeOptionSerializer(serializers.ModelSerializer):
    class Meta:
        ...
        fields = [
            'id', 'label', 'code', 'pricing_mode', 'price_value', 'currency', 
            'order', 'is_default', 'swatch_hex', 'icon', 'qty_attr_code'
        ]

    def validate_code(self, value):
        attribute = self.context.get('attribute')
        if attribute and AttributeOption.objects.filter(
            attribute=attribute, code=value
...
class TemplateAttributeSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = [
            'id', 'name', 'code', 'type', 'is_required', 'order', 'render_variant',
    ...
    def validate_code(self, value):
        ...
        if template and TemplateAttribute.objects.filter(
...
class ProductTemplateSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = [
            'id', 'product_class', 'line_name', 'code', 'base_price_net', 
            'currency', 'requires_dimensions', 'is_active', 'version', 
...
class ProductTemplateListSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = ProductTemplate
        fields = [
            'id', 'product_class', 'line_name', 'code', 'version', 
            'is_active', 'created_at', 'attributes_count'
        ]
